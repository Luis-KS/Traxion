({  
    ...
    searchRecordsBySearchPhrase : function (component) { 
        ...
        if (!$A.util.isEmpty(searchPhrase)) {
            ...
            let filteredData = allData.filter(function(record){ 
                const {Folio_de_Pedido__c = null} = record;  
                if(Folio_de_Pedido__c){ 
                    return Folio_de_Pedido__c.includes(searchPhrase);
                } 
            });
    ...
    searchOLIBySelection : function (component, pedido) { 
        ...
        const {Folio_de_Pedido__c=null, ID_de_Pedido__c=null, Mostrar_Envio_a_Autorizacion__c=null} = pedido;
        ...
        component.set('v.showHideAutorizacion', !Mostrar_Envio_a_Autorizacion__c);
    ...
    getDPNliInformation : function(component, pedido) {
        ...
        console.log("Inside renderTitulo");
        ...
        action.setCallback(this, function(response) { 
            ...
            if (state === "SUCCESS") {
                ...
                Order_Line_Items__r.forEach(function(oli) {
                    const matchingDpnli = responseVal.find(function(dpnli) {
                        return oli.Product__c === dpnli.Product__c;
                    });
                    if (matchingDpnli) {
                        oli.DPNAvailability = matchingDpnli.L_mite_Mensual__c || 'N/A';
                        oli.DPNValidado = matchingDpnli.L_mite_Mensual__c && matchingDpnli.Consumido__c ? matchingDpnli.Consumido__c : matchingDpnli.L_mite_Mensual__c ? 0 : 'N/A';
                        oli.DPNDisponible = matchingDpnli.L_mite_Mensual__c && matchingDpnli.Consumido__c ? matchingDpnli.L_mite_Mensual__c - matchingDpnli.Consumido__c : matchingDpnli.L_mite_Mensual__c ? matchingDpnli.L_mite_Mensual__c : 'N/A';
                    } else {
    ...
    updateOLI : function(component, oliId, actionType, quantity, message) {
        ...
        action.setCallback(this, function(response) { 
            ...
            if (state === "SUCCESS") {
                ...
                component.set('v.showHideAutorizacion', !responseVal[0].Mostrar_Envio_a_Autorizacion__c); 
